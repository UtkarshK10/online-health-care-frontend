...
const ConfirmationPage = () => {
    ...
    const handleNewAddress = () => {
        axios.get('/api/address/address_count', {
            headers: {
            ...
            .then(res => {
                ...
                address_count === ALLOWED_ADDRESSES ? M.toast({ html: "Address limit reached, please delete some to add new!" }) : setNewAddress(true);
            })
            .catch(e => {
    ...
    const prePopulate = () => {
        ...
        if (zip_code.length === 6) {
            axios.get(`https://cors-anywhere.herokuapp.com/http://www.postalpincode.in/api/pincode/${zip_code}`)
                .then(res => {
                    const data = res.data.PostOffice[0];
                    setAddress({ ...address, state: data['State'], city: data['District'] })
                })
                .catch(e => {
                    console.log(e);
                })
                            ...
                            <div className="card-tabs">
                                <ul className="tabs tabs-fixed-width">
                                    {oldAddresses.map((address, idx) => {
                                        return (
                                            <li className="tab" key={address.id}><a className="pcolour" href={`#${address.id}`}><h6 style={{ color: "white" }}>Address #{idx + 1}</h6></a></li>
                                        )
                                    })}
                                </ul>
                            </div>
                            <div className="card-content grey lighten-3" style={{ color: "black" }}>
                                        ...
                                        <div className='col s12 m6 l6'>
                                            <div className='input-field'>
                                                <input
                                                    ...
                                                    className='validate'
                                                />
                                                <label htmlFor='city' className={`${address.city && "active "}font-app`}>
                                                    City
                      </label>
                                            </div></div>
                                    ...
                                    <div className='row'>
                                        <div className='input-field'>
                                            <input
                                                ...
                                                className='validate'
                                            />
                                            <label htmlFor='state' className={`${address.state && "active "}font-app`}>
                                                State
                      </label>
                                        </div>
                                    </div>
