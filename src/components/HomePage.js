...
const HomePage = () => {
  ...
  const fetchDoctors = useCallback(() => axios.get('/api/doctors/', {
    headers: {
  ...
  const makeAppointment = id => {
    if (auth.credits < 10) {
      M.toast({ html: 'You don\'t have enough credits, please add and then continue' })
      return;
    }
  ...
  return <>
        ...
        {currPageDoctors.map(doctor => {
          ...
          return (
            < div key={doctor.id} className='col s12 m6 l4' >
              <DoctorCard key={doctor.id} id={id} name={name} photo={profile_url} experience={experience} speciality={speciality} appointment={makeAppointment}
    ...
    <Pagination doctorsPerPage={doctorsPerPage}
